...
class Conv_dw(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, stride=1, bias=False, norm=False, relu=True, transpose=False,
                 relu_method=nn.ReLU, norm_method=nn.BatchNorm2d, groups=1, simam=False, simam_e_lambda=1e-4):
        super(Conv_dw, self).__init__()
        self.transpose = transpose
        self.use_simam_feature_attention = simam
        self.simam_e_lambda = simam_e_lambda
        self.simam_activation = nn.Sigmoid()

        layers = []
        if transpose:
            use_bias = bias and not norm
            padding = kernel_size // 2 - 1
            layers.append(
                nn.ConvTranspose2d(in_channel, out_channel, kernel_size, padding=padding, stride=stride, bias=use_bias)
            )
        else:
            padding = kernel_size // 2
            use_bias_dw = bias and not norm
            layers.append(
                nn.Conv2d(in_channel, in_channel, kernel_size, padding=padding, stride=stride,
                          groups=in_channel, bias=use_bias_dw)
            )
            use_bias_pw = bias and not norm
            layers.append(
                nn.Conv2d(in_channel, out_channel, kernel_size=1, bias=use_bias_pw)
            )
    ...
    def forward(self, x):
        x = self.conv_seq(x)
        if self.training and self.use_simam_feature_attention:
            b, c, h, w = x.size()
            n = w * h - 1 if w * h > 1 else 1e-8
            x_minus_mu_square = (x - x.mean(dim=[2, 3], keepdim=True)).pow(2)
            y = x_minus_mu_square / (4 * (x_minus_mu_square.sum(dim=[2, 3], keepdim=True) / n + self.simam_e_lambda)) + 0.5
            attention_weights = self.simam_activation(y)
